...
- Shell 脚本（`.sh`, `.bash`）
- **代码说明文档**：解释架构、API、模块职责、使用方法、命令行参数等（如 README、docstring 生成的文档、API reference）
- 测试代码（`tests/`）
- 类型定义、协议接口、常量定义
- Package 配置文件（`setup.py`, `setup.cfg`, `pyproject.toml`）

### 禁止推送（Forbidden）

- **实验方案文档**：包含研究假设、消融设计、对比策略、实验矩阵
- **实验结果/报告**：指标数值、训练曲线分析、质量评估报告、TSTR 结果、消融结论
- 训练产物（checkpoint `*.pt`, 特征 `*.pkl`, 日志 `*.log`）— 已被 `.gitignore` 覆盖
- 数据集文件、原始数据
...
**例子**：
- ✅ 可以推：`experiments/README.md`（解释调度脚本怎么用）
- ❌ 禁止推：`synthetic_data_quality_report_a.md`（合成数据质量评估结果）
- ✅ 可以推：trainer 中新增 loss 函数的 docstring
- ❌ 禁止推：某次训练的 loss 曲线分析和结论
