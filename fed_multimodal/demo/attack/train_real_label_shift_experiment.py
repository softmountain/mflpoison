...
MODE_DISPLAY_NAMES = {
    ...
    'real_same_group_label_shift': {
        'zh': '组内真实特征标签攻击',
        'en': 'Intra-group real-feature label-shift attack',
    },
}
...
def parse_args():
    parser = argparse.ArgumentParser(description='Real-feature label-shift control experiment for demo federated learning')
    parser.add_argument('--exp_name', type=str, default='ucf101_real_label_shift_100r')
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='*',
        default=['clean', 'real_same_group_label_shift'],
        choices=['clean', 'real_same_group_label_shift'],
    )
...
def plot_curves(metrics, save_path: Path):
    ...
    plt.title('Real-Feature Label-Shift Evaluation Curves')
    plt.legend()
...
def _build_poisoned_metrics(
    ...
    attack_metrics = compute_attack_success_metrics(truth, pred, attacked_label_ids, same_group_target_map)
...
def run_mode(
    ...
    for epoch in range(int(args.num_epochs)):
        ...
        for client_id in selected_clients:
            if mode == 'real_same_group_label_shift' and client_id in malicious_client_set:
                client = ClientRealSameGroupLabelShift(
                    args,
                    device,
                    criterion,
...
def main():
    ...
    print(f'Saved real-label-shift metrics to {output_dir / "metrics.json"}')
