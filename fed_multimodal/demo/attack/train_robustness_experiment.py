...
from .gan_source import load_demo_gan
...
from .robustness_client import ClientRobustnessShift
from .visual_groups import (
    MODE_DISPLAY_NAMES,
    attacked_label_ids_from_groups,
    build_cross_group_target_map,
    build_group_to_label_ids,
    build_label_to_attack_group,
    build_same_group_candidate_map,
    build_same_group_target_map,
    label_report,
...
MODE_TARGET_KIND = {
    'clean': 'same_group',
    'gan_same_group_shift': 'same_group',
    'cross_modal_mismatch_eval': 'cross_group',
}
DEFAULT_ATTACK_OUTPUT_ROOT = Path('/home/xp/fed-multimodal/fed_multimodal/demo/results/ucf101/attack')


def parse_args():
    parser = argparse.ArgumentParser(description='Robustness evaluation for demo federated learning')
    parser.add_argument('--exp_name', type=str, default='robustness_eval')
    ...
    parser.add_argument('--gan_checkpoint', type=str, default='/home/xp/fed-multimodal/fed_multimodal/results/demo/ucf101/gan/checkpoints/ckpt_200_0309BASE_STRONGT.pt')
    parser.add_argument('--teacher_checkpoint', type=str, default='/home/xp/fed-multimodal/fed_multimodal/results/demo/ucf101/training/fold1_fed_avg_sr02_ep200_lr005_glr001_hid128_best_model.pt')
    parser.add_argument('--attack_label_spec_version', type=str, default='v2')
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='*',
        default=['clean', 'gan_same_group_shift', 'cross_modal_mismatch_eval'],
        choices=['clean', 'gan_same_group_shift', 'cross_modal_mismatch_eval'],
    )
...
def _mode_target_map(mode: str, same_group_target_map, cross_group_target_map):
    if MODE_TARGET_KIND[mode] == 'cross_group':
        return cross_group_target_map
    return same_group_target_map


def _build_poisoned_metrics(
    mode: str,
    detailed_result: dict,
    attacked_label_ids,
    same_group_target_map,
    cross_group_target_map,
    label_to_group_id,
    group_to_label_ids,
    class_names,
    clean_reference: dict = None,
):
    truth = detailed_result['truth']
    pred = detailed_result['pred']
    target_map = _mode_target_map(mode, same_group_target_map, cross_group_target_map)
    ...
    group_metrics = compute_same_group_attack_success_metrics(
        truth,
        pred,
        attacked_label_ids,
        label_to_group_id,
    ...
    if clean_reference is not None:
        clean_target_map = _mode_target_map(mode, same_group_target_map, cross_group_target_map)
        clean_attack_metrics = compute_attack_success_metrics(
            clean_reference['truth'],
            clean_reference['pred'],
            attacked_label_ids,
            clean_target_map,
        )
        clean_group_metrics = compute_same_group_attack_success_metrics(
            clean_reference['truth'],
            clean_reference['pred'],
            attacked_label_ids,
            label_to_group_id,
        ...
        poisoned_metrics['attack_success_wrong_rate_increase_vs_clean'] = float(
            attack_metrics['attack_success_wrong_rate'] - clean_attack_metrics['attack_success_wrong_rate']
        )
        poisoned_metrics['attack_success_target_hit_rate_increase_vs_clean'] = float(
            attack_metrics['attack_success_target_hit_rate'] - clean_attack_metrics['attack_success_target_hit_rate']
        )
        poisoned_metrics['same_group_other_hit_rate_increase_vs_clean'] = float(
            group_metrics['same_group_other_hit_rate'] - clean_group_metrics['same_group_other_hit_rate']
        )
        poisoned_metrics['impact_on_non_attacked_classes'] = compute_non_attacked_impact(
            clean_reference['main_task_success_excluding_attacked'],
            main_task,
        )
    else:
...
def _save_mode_outputs(
    ...
    group_confusions = build_group_confusion_outputs(
...
def _load_clean_reference(clean_reference_dir: Path):
    ...
    clean_truth = [int(row['truth_id']) for row in predictions]
    clean_pred = [int(row['pred_id']) for row in predictions]
    return {
        'truth': clean_truth,
        'pred': clean_pred,
...
def _save_heatmaps(output_dir: Path, confusion_by_mode: dict, class_names: list, modes: list):
    heatmap_dir = output_dir / 'heatmaps'
    if 'clean' in confusion_by_mode:
        plot_confusion_heatmap(
            confusion_by_mode['clean']['full']['row_normalized'],
            class_names,
            'Clean baseline confusion matrix (row normalized, %)',
            heatmap_dir / 'confusion_clean_normalized.png',
            cmap='magma',
            vmin=0.0,
            vmax=100.0,
        )
        plot_group_confusion_heatmaps(
            confusion_by_mode['clean']['groups'],
            'Clean baseline',
            heatmap_dir / 'groups' / 'clean',
            cmap='magma',
        )
    for mode in modes:
        if mode == 'clean':
        ...
        display = MODE_DISPLAY_NAMES.get(mode, {'en': mode})['en']
        plot_confusion_heatmap(
            confusion_by_mode[mode]['full']['row_normalized'],
            class_names,
            f'{display} confusion matrix (row normalized, %)',
            heatmap_dir / f'confusion_{mode}_normalized.png',
            cmap='magma',
            vmin=0.0,
            vmax=100.0,
        )
        plot_group_confusion_heatmaps(
            confusion_by_mode[mode]['groups'],
            display,
            heatmap_dir / 'groups' / mode,
            cmap='magma',
        )
        if 'clean' in confusion_by_mode:
            plot_confusion_difference_heatmap(
                ...
                confusion_by_mode['clean']['full']['row_normalized'],
                class_names,
                f'{display} minus clean confusion matrix (row normalized, %)',
                heatmap_dir / f'confusion_{mode}_minus_clean.png',
            )
            plot_group_confusion_difference_heatmaps(
                confusion_by_mode[mode]['groups'],
                confusion_by_mode['clean']['groups'],
                f'{display} minus clean',
                heatmap_dir / 'groups' / f'{mode}_minus_clean',
            )
...
def run_mode(
    ...
    for epoch in range(int(args.num_epochs)):
        ...
        malicious_selected = [client_id for client_id in selected_clients if client_id in malicious_client_set]
        client_sampling['rounds'].append({
            'round': epoch + 1,
            'selected_client_ids': selected_clients,
            'malicious_selected_client_ids': malicious_selected,
        })

        for client_id in selected_clients:
            if mode != 'clean' and client_id in malicious_client_set:
                client = ClientRobustnessShift(
                    args,
                    device,
                    criterion,
                    dataloader_dict[client_id],
                    copy.deepcopy(server.global_model),
                    gan,
                    mode=mode,
                    attack_label_ids=attacked_label_ids,
                    same_group_target_map=same_group_target_map,
                    cross_group_target_map=cross_group_target_map,
                    same_group_candidate_map=same_group_candidate_map,
                    client_id=client_id,
                    round_idx=epoch,
                )
            else:
...
def main():
    ...
    same_group_candidate_map = build_same_group_candidate_map(group_spec, attack_enabled_only=True)
    attacked_label_ids = attacked_label_ids_from_groups(group_spec)
    same_group_target_map = build_same_group_target_map(attacked_label_ids)
    cross_group_target_map = build_cross_group_target_map(attacked_label_ids)
    for label_id in attacked_label_ids:
        label_id = int(label_id)
        if label_id in cross_group_target_map:
            continue
        source_group = label_to_group_id[label_id]
        for group_id, label_ids in group_to_label_ids.items():
            if group_id != source_group and label_ids:
                cross_group_target_map[label_id] = int(label_ids[0])
                break
    attacked_report = label_report(
        attacked_label_ids=attacked_label_ids,
        same_group_target_map=same_group_target_map,
        cross_group_target_map=cross_group_target_map,
        spec_version=args.attack_label_spec_version,
        group_spec=group_spec,
        same_group_candidate_map=same_group_candidate_map,
    )
    save_json(attacked_report, output_dir / 'attacked_label_report.json')
    if args.validate_groups_only:
        print(f'Validated attack group spec: {len(group_spec["groups"])} groups, {len(attacked_label_ids)} attacked labels')
        return
    ...
    if args.clean_reference_dir:
        clean_reference = _load_clean_reference(Path(args.clean_reference_dir))
    comparison_rows = []
    confusion_by_mode = {}
    ...
    for mode in selected_modes:
        ...
        poisoned_metrics = _build_poisoned_metrics(
            mode,
            detailed_result,
            attacked_label_ids,
            same_group_target_map,
        ...
        confusion_outputs = _save_mode_outputs(
            mode,
            output_dir,
            detailed_result,
            poisoned_metrics,
    ...
    print(f'Saved robustness metrics to {output_dir / "metrics.json"}')
