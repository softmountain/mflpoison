...
def data_partition(args: dict):
    ...
    for fold_idx in range(len(train_file_list)):
        ...
        print(f"Fold {fold_idx}: Train split lines: {len(train_split_files)}, Test split lines: {len(test_split_files)}")
        print(f"Total available files: {len(file_id_list)}")
        if len(file_id_list) > 0:
            print(f"Sample file_id: {file_id_list[0]}")
        if len(test_split_files) > 0:
            print(f"Sample test line: {test_split_files[0].strip()}")
