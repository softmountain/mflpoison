...
def setup_file_logger(log_dir: Path, log_prefix: str = 'train') -> Path:
    """Attach a file handler so every log is persisted to disk."""
    log_dir = Path(log_dir)
    log_dir.mkdir(parents=True, exist_ok=True)
    timestamp = time.strftime('%Y%m%d_%H%M%S')
    log_path = log_dir.joinpath(f'{log_prefix}_{timestamp}.log')
...
def parse_monitor_labels(raw_value):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='comma-separated label ids to monitor per-class accuracy on dev/test'
    )
    ...
    args.monitor_labels = parse_monitor_labels(args.monitor_labels)
    return args
...
if __name__ == '__main__':
    ...
    log_file = setup_file_logger(log_dir, log_prefix=f'{args.dataset}_{args.fed_alg}_base')
