...
class Server(object):
    ...
    def inference(self, dataloader):
        ...
        monitor_labels = getattr(self.args, 'monitor_labels', None)
    ...
    def log_classification_result(
        ...
        if data_split == 'train':
            ...
            label_acc = self.result_dict[self.epoch][data_split].get('monitored_label_acc')
        if data_split == 'train':
            label_acc = None
        ...
        if label_acc and data_split in ['dev', 'test']:
            readable = []
            for label, value in label_acc.items():
                if value != value:  # NaN check
                    readable.append(f'label {label}: N/A (no samples)')
                else:
                    readable.append(f'label {label}: {value:.2f}%')
            logging.info(f'{data_split} monitored labels => ' + '; '.join(readable))
        # loggin to folder
    ...
    def summarize_dict_results(self):
        ...
        if not self.multilabel:
            ...
            if 'monitored_label_acc' in self.best_test_dict:
                result['monitored_label_acc'] = self.best_test_dict['monitored_label_acc']
        else:
