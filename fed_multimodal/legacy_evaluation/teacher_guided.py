...
from fed_multimodal.legacy_evaluation.checkpoint import load_legacy_checkpoint
from fed_multimodal.legacy_evaluation.data import (
    add_partition_arguments,
    evaluation_loader_from_args,
)
from fed_multimodal.poison_gan.kplus1 import load_teacher_model
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='artifact/legacy_evaluation/teacher_guided')
...
def load_evaluation_checkpoint(gan, path, checkpoint):
    """Load generators and only expose joint metrics backed by saved weights."""
    joint_metric_available = 'joint_discriminator' in checkpoint
    gan.load_checkpoint(
        path,
        load_discriminators=joint_metric_available,
    )
    return joint_metric_available
...
def evaluate_core(
    ...
    num_batches,
    device,
    joint_metric_available=True,
):
    gan.audio_generator.eval()
    gan.video_generator.eval()
    if joint_metric_available:
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            if joint_metric_available:
                ...
                joint_gap_curve.append(
                    (joint_real_val.mean() - joint_fake_val.mean()).item()
                )
    ...
    if joint_metric_available:
        joint_real = torch.cat(joint_real_list, dim=0).mean().item()
        joint_fake = torch.cat(joint_fake_list, dim=0).mean().item()
        joint = {
            'available': True,
        ...
        joint = {
            'available': False,
            'reason': 'checkpoint does not contain joint_discriminator weights',
        }
...
def main():
    ...
    data, dataloader = evaluation_loader_from_args(args)

    # Teacher
    teacher, _ = load_teacher_model(
        model_path=args.model_path,
        num_classes=data.num_classes,
        audio_input_dim=data.audio_feat_dim,
        video_input_dim=data.video_feat_dim,
    ...
    ckpt = load_legacy_checkpoint(
        args.checkpoint, 'teacher_guided', map_location=device
    )
    ...
    joint_metric_available = load_evaluation_checkpoint(
        gan,
        args.checkpoint,
        ckpt,
    )
    ...
    results = evaluate_core(
        ...
        args.num_batches,
        device,
        joint_metric_available=joint_metric_available,
    )
    ...
    results['meta'] = {
        ...
        'partition': 'client' if args.client_id is not None or args.use_train else args.partition,
    ...
    if results['joint']['available']:
        logging.info(
            f"Joint Gap: {results['joint']['gap']:.4f} "
            f"(real={results['joint']['real_logit_mean']:.3f}, "
            f"fake={results['joint']['fake_logit_mean']:.3f})"
        )
    else:
        logging.info("Joint Gap: unavailable (checkpoint has no joint discriminator)")
