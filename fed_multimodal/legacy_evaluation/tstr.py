...
from mflpoison.artifacts import load_synthetic
...
def parse_args():
    ...
    add_partition_arguments(parser, allow_client=False)
    ...
    parser.add_argument('--val_split', type=float, default=0.1,
                        help='Deprecated and ignored; the fixed FedMM dev partition is used')
...
def fit_tstr(
    ...
    """Train on synthetic data, select on dev, and touch test exactly once."""
    ...
    for epoch in range(1, args.num_epochs + 1):
        train_loss, train_acc, _, _ = train_epoch_fn(
            model, synth_loader, criterion, optimizer, device, num_classes
        )
        val_loss, val_acc, val_uar, val_f1 = evaluate_fn(
            model, dev_loader, criterion, device, num_classes
        )
    ...
    return best_val_acc, test_metrics, history
...
def main():
    ...
    canonical = load_synthetic(args.synthetic_data)
    ...
    val_loader = dm.get_loader('dev')
    test_loader = dm.get_loader('test')
    ...
    best_val_acc, test_metrics, history = fit_tstr(
        model=model,
    ...
    test_loss, test_acc, test_uar, test_f1 = test_metrics
